...
# maps 폴더에 이미 생성된 맵 중 하나를 시작 전에 선택한다.
...
def find_weather_data_file(config_file, files):
    # weather_data가 map_config.json에 지정되어 있으면 해당 파일을 우선 사용한다.
    # 지정되어 있지 않으면 선택한 지도 폴더 안의 유일한 CSV 파일을 자동으로 사용한다.
...
# 소방관, 소방차, 방화선의 실제 기능은 1.4 설계가 확정된 뒤 구현한다.
TOOL_FIREFIGHTER = 1
TOOL_TRUCK = 2
TOOL_HELI = 3
TOOL_PLANE = 4
...
def get_suppression_marker_cells(tool_id, gx, gy):
    ...
    if tool_id in (TOOL_FIREBREAK, TOOL_FIREFIGHTER):
...
def get_suppression_marker_style(tool_id):
    ...
    if tool_id == TOOL_FIREFIGHTER:
        return SUPPRESSION_PENDING_BLUE, 95
    return WHITE, 95
...
def draw_ground_suppression_preview():
    if selected_tool not in (TOOL_FIREBREAK, TOOL_TRUCK):
        return
    ...
    for tx, ty in target_cells:
        rect = pygame.Rect(tx * CELL_SIZE, ty * CELL_SIZE, CELL_SIZE, CELL_SIZE)
        screen.blit(preview_overlay, rect.topleft)
        pygame.draw.rect(screen, WHITE if selected_tool == TOOL_FIREBREAK else TRUCK_ACTIVE_GREEN, rect, 1)
...
def draw_sidebar(leaf_count, stem_count):
    ...
    y += heading_gap
    draw_text("Firebreak / Truck / Heli / Plane implemented.", x0, y, TEXT_MUTED, use_small=True)
    y += small_line_h + 3
...
def handle_grid_click(mx, my, mouse_button):
    ...
    if mouse_button == 1:
        ...
        # 아직 기능이 확정되지 않은 소방관은 효과를 적용하지 않는다.
